...
void dynaPlots::plotMainSignals(std::vector<double> &outer,
    ...
    cvui::text(     frame,            graphX, graphY * step + topOffset + graphDY, "Channel 2 : Noise (y[n])");
    ...
    cvui::text(     frame,            graphX, graphY * step + topOffset + graphDY, "Channel 1 : Raw Signal (x[n])");
    ...
    cvui::text(     frame,            graphX, graphY * step + topOffset + graphDY, "Canceller signal (z[n]) ");
    ...
    cvui::text(     frame,            graphX, graphY * step + topOffset + graphDY, "DNF output (e[n])");
    ...
    cvui::text(     frame,            graphX, graphY * step + topOffset + graphDY, "LMS canceller");
...
void dynaPlots::plotTitle(std::string title, long count, int duration,const char* fullPath){
  cvui::printf(frame, gapX,           titleY, "Recording %s -- Sample number: %ld , Duration: %d [min] %d [s], %s", 
